...
class Gs500001Spider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in range(3):
            if i == 0:
                url = 'http://www.gs5000.com/huangdi/hanchao/index.html'
                req = scrapy.Request(url=url, callback=self.parse, dont_filter=True)
                yield req
            else:
                url = f'http://www.gs5000.com/huangdi/hanchao/index_{i + 1}.html'
                req = scrapy.Request(url, callback=self.parse, dont_filter=True)
                yield req

    def parse(self, response):
        get_info = response.xpath('//ul[@class="e2"]/li/a[2]/@href').extract()
    ...
    def detail_parse(self, response):
        ...
        issue_time = response.xpath('//div[@class="info"]/text()').extract()[1]
        source = response.xpath('//div[@class="info"]/a[1]/text()').extract_first()
        content = response.xpath('//div[@id="newstext"]').extract_first()
        tags = response.xpath('//div[@class="a_tags"]/a/text()').extract()
        images_url = response.xpath('//div[@id="newstext"]//img/@src').extract()
        item = InfoItem()
        item['images'] = []
        images = []
        if images_url:
...
if __name__ == '__main__':
    ...
    cmdline.execute(['scrapy', 'crawl', 'Gs5000_01'])
