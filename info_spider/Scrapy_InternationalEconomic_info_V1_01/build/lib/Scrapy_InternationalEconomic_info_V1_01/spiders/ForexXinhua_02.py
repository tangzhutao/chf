...
class InternationalEconomicSpider(scrapy.Spider):
    ...
    def start_requests(self):
        # for i in range(3, 36):
        #     url = f'http://app.xinhua08.com/prop.php?pid=2%2C3&cid=5273&page={i + 1}'
        #     req = scrapy.Request(url=url, callback=self.parse)
        #     yield req
        url = 'http://world.xinhua08.com/'
        req = scrapy.Request(url=url, callback=self.parse, dont_filter=True)
        yield req

    def parse(self, response):
        get_info = response.xpath('//div[@id="latest"]/article/div[@class="newsinfo"]/a/@href').extract()
        # print(get_info)
    ...
    def detail_parse(self, response):
        ...
        item['information_source'] = '新华财经-国际财经'
        item['sign'] = '19'
...
if __name__ == '__main__':
    ...
    cmdline.execute(['scrapy', 'crawl', 'ForexXinhua_02'])
