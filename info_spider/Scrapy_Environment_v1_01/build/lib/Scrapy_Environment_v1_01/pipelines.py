...
from Scrapy_Environment_v1_01.MysqlPool import MysqlPool
...
class MongoPipeline(object):
    ...
    def __init__(self):
        ...
        # self.collection_name = self.st.get('MONGO_COLLECTION_TEST')
        self.count = 0
        self.menu_count = 0
        self.client = None
        self.db = None
    ...
    def close_spider(self, spider):
        ...
        config.SpiderLog('环保经济 资讯', spider.name, update_time, self.count, self.menu_count, spider.base_url, spider.url_name)
        self.client.close()
    ...
    def process_item(self, item, spider):
        ...
        if self.db[self.collection_name].count_documents(
            ...
            self.count += 1
            update = 'the date is update: ' + item['title'] + 'PublishTime：' + item['issue_time']
            logger.info(update)
            return
        else:
