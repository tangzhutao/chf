...
import json
import time
from Scrapy_D44_robodata_V1_02.mysqlAPI import select_count, select, insert, check_table
...
class D44Aimei01Spider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for k, v in root_id.items():
            ...
            if not select(menu_name, parent_menu_id, isRep):
                ...
                v = select(menu_name, parent_menu_id, isRep)['menu_id']
    ...
    def parse1(self, response):
        ...
        for info in config_info:
            n = select_count(response.meta['parent_id']) + 1
            parent_menu_id = response.meta['parent_id']
            name = info['name']
            child = info['child']  # 判断是否还有子数据，若为空，则没有子数据
            menu_id = parent_menu_id + "{:03d}".format(n)
            menu_name = name
            if not select(menu_name, parent_menu_id, info['id']):
                insert(menu_id, menu_name, parent_menu_id, info['id'])
            else:
                menu_id = select(menu_name, parent_menu_id, info['id'])['menu_id']
            if child:
    ...
    def rec(self, parent_menu_id, info):
        for data in info:
            n = select_count(parent_menu_id) + 1
            menu_name = data['name']
            menu_id = parent_menu_id + "{:03d}".format(n)
            if not select(menu_name, parent_menu_id, data['id']):
                insert(menu_id, menu_name, parent_menu_id, data['id'])
            else:
                menu_id = select(menu_name, parent_menu_id, data['id'])['menu_id']
