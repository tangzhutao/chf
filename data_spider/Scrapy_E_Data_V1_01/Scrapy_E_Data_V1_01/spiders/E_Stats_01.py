...
class EStats01Spider(scrapy.Spider):
    name = 'E_Stats_01'
    ...
    # 从1990年开始爬取
    # u=1; experience=show; _trs_uv=kc1qy830_6_13w3; JSESSIONID=E328658823CF0008DBB8E205B67F1EB0
    # COOKIE = {
    #     "JSESSIONID": "E328658823CF0008DBB8E205B67F1EB0",
    #     "experience": "show",
    #     "u": "1",
    ...
    def start_requests(self):
        ...
        p_name = '国家统计局'
        isRep = '建筑业'
        if not mysql_config.select(p_name, p_id, isRep):
            n = mysql_config.select_count(p_id) + 1
            m_id = p_id + "{:03d}".format(n)
            mysql_config.insert(m_id, p_name, p_id, isRep)
        else:
            m_id = mysql_config.select(p_name, p_id, isRep)['menu_id']

        for k, v in self.data.items():
            n = mysql_config.select_count(m_id) + 1
            menu_id = m_id + "{:03d}".format(n)
            isRep = v['id']
            if not mysql_config.select(k, m_id, isRep):
                mysql_config.insert(menu_id, k, m_id, isRep)
            else:
                menu_id = mysql_config.select(k, m_id, isRep)['menu_id']
            ...
            if info['isParent']:
                ...
                if info['dbcode'] in ['fsjd', 'fsnd']:
                    for code in self.ProvinceCode:
                        ...
                        req = scrapy.Request(url=url, callback=self.data_parse,
                                             dont_filter=True,
                    ...
                    param = urlencode(keyvalue).replace('+', '')
                    url = self.base_url + '?' + param
                    req = scrapy.Request(url=url, callback=self.data_parse, dont_filter=True,
                                         meta={'parent_id': menu_id})
                    yield req
...
if __name__ == '__main__':
    ...
    args = "scrapy crawl E_Stats_01".split()
