...
MYSQL_PASSWORD = str(settings.get("MYSQL_PASSWORD"))
MYSQL_DATABASE = settings.get("MYSQL_DATABASE")
MYSQL_CHRASET = settings.get("MYSQL_CHRASET")
# MYSQL_TABLE = settings.get("MYSQL_TABLE")
MYSQL_TABLE_REPO = settings.get("MYSQL_TABLE_REPO")
...
class Mysql(object):
    ...
    def check_table(self):
        ...
        if MYSQL_TABLE_REPO not in table_list:
    ...
    def select_count(self, parent_id):
        sql = "SELECT * FROM %s where parent_menu_id = '%s'" % (MYSQL_TABLE_REPO, parent_id)
    ...
    def select(self, menu_name, parent_menu_id, isRep):
        sql = "SELECT * FROM %s where menu_name = '%s' and parent_menu_id = '%s' and isRep = '%s'" % (
            MYSQL_TABLE_REPO, menu_name, parent_menu_id, isRep)
        count = self._cursor.execute(sql)
    ...
    def insert(self, menu_id, menu_name, parent_id, isRep):
        try:
            ...
            sql = "insert into %s(menu_id, menu_name, parent_menu_id, isRep) values('%s','%s','%s','%s')" % (
                MYSQL_TABLE_REPO, menu_id, menu_name, parent_id, isRep)
    ...
    # 日志：记录菜单新增量
    def menu_count(self):
        sql = "select * from %s" % (MYSQL_TABLE_REPO)
        count = self._cursor.execute(sql)
        return count
